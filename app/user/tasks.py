...
def admin_marketplace_notify():
    ...
    for order in response['orders']:
        if str(datetime.now().date()) == order['date'] and datetime.fromtimestamp(float(order['timestamp'])).time().minute == datetime.now().time().minute and datetime.fromtimestamp(float(order['timestamp'])).time().hour == datetime.now().time().hour:
